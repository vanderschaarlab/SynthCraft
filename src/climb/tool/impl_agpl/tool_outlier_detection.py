...
def cleanlab_outlier_detection(
    ...
    # Flag data points with OOD scores above the below the low threshold
    ...
    plt.title("Distribution of Averaged OOD Scores with High and Low Thresholds")
    ...
    cleaned_X = X[mask_inliers]
    cleaned_y = y[mask_inliers]
    outliers_X = X[mask_outliers]  # noqa: F841
    outliers_y = y[mask_outliers]

    # Save the Cleaned Data to CSV
    feature_columns = df.columns[df.columns != target_variable]
    df_cleaned = pd.DataFrame(cleaned_X, columns=feature_columns)
    df_cleaned[target_variable] = cleaned_y
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "task_type": {
                            "type": "string",
                            "enum": ["classification", "regression", "survival_analysis"],
                            "description": "Type of problem (classification, regression or survival_analysis).",
                        },
                    },
